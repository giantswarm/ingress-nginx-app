...
`cluster.profile` | Cluster usage profile. Dynamically calculated during cluster creation. Supported values are `1` for extra small, `2` for small, and currently any value higher than 2 when actual cluster profile is larger than small. HPA and PDB are disabled, and resource requests unset for extra small clusters. On small clusters some small resource requests are made, while HPA and PDB are still disabled. On larger than small clusters HPA and PDB are enabled by default, and non-trivial resource requests set for some out-of-the-box guaranteed capacity. | `3`
